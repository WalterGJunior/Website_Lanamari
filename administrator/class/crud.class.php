  ...
  */
  class crud
  {
	...
	public function inserir($campos, $valores) // funçao de inserçao, campos e seus respectivos valores como parametros
	{
		...
		{
			die ("<div class='error'>Erro na inclusão " . '<br>Linha: ' . __LINE__ . "<br>" . mysql_error() . "</div>");
		}else{
			if($_REQUEST[pag])
			{
			print "<script>parent.window.location='$_REQUEST[pag]insert=success#UP'</script>";
		}
		else
		{
			print "<script>parent.window.location='index.php'</script>";
		}
		}
	}
	...
	public function atualizar($camposvalores, $where = NULL) // funçao de ediçao, campos com seus respectivos valores e o campo id que define a linha a ser editada como parametros
	{
		...
		{
			die ("<div class='error'>Erro na atualização " . "<br>Linha: " . __LINE__ . "<br>" .mysql_error() . "</div>");
		}else{
			if($_REQUEST[pag])
			{
			print "<script>parent.window.location='$_REQUEST[pag]update=success#UP'</script>";
		}
		else
		{
			print "<script>parent.window.location='index.php'</script>";
	...
	public function excluir($where = NULL) // funçao de exclusao, campo que define a linha a ser editada como parametro
	{
	  ...
	  if ($regs > 0){
		...
		{
			die ("<div class='error'>Erro na exclusão " . '<br>Linha: ' . __LINE__ . "<br>" .mysql_error() ."<br><a href='index.php'>Voltar ao Menu</a></div>" );
		}else{
			if($_REQUEST[pag])
			{
			print "<script>parent.window.location='$_REQUEST[pag]delete=success#UP'</script>";
		}
		else
		{
			print "<script>parent.window.location='index.php'</script>";
			...
			print "<div class='error'>Registro Não encontrado!</div>";
	  }
	}  	

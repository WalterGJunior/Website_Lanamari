...
if(@$getId){ //se existir recupera os dados e tras os campos preenchidos
    $consulta = mysql_query("SELECT * FROM tb_produtos_categoria WHERE tx_id = + $getId");
...
if(isset($_POST['cadastrar'])){  // caso nao seja passado o id via GET cadastra 
    ...
    $_POST['tx_categoria_nome'] = utf8_decode($_POST['tx_categoria_nome']);

$crud = new crud('tb_produtos_categoria');  // instancia classe com as operaçoes crud, passando o nome da tabela como parametro
$crud->inserir("tx_categoria_nome","'$_POST[tx_categoria_nome]'"); // utiliza a funçao INSERIR da classe crud        
...
$crud = new crud('tb_produtos_categoria'); // instancia classe com as operaçoes crud, passando o nome da tabela como parametro
$crud->atualizar("tx_categoria_nome='$_POST[tx_categoria_nome]'", "tx_id='$getId'"); // utiliza a funçao ATUALIZAR da classe crud
...
<form action="" method="post"><!--   formulario carrega a si mesmo com o action vazio  -->
    <input type="hidden" name="pag" value="products_category_list.php?tx_id=<?php echo @$getId; ?>&" /><!--   identifica a pagina de requisição para o crud  -->

    <!--   trazendo campos preenchidos caso esteja no modo de ediçao  -->
    <div class="CSSTableGenerator" >  
        <table>
            <tr>
                <td class="backgroundCenter">Categorias</td>
                <td></td>
            </tr>
            <tr>
                <td class="backgroundCenter">Nome da Categoria</td>
                <td><input class="campoHidden" required type="text" size="40%"  name="tx_categoria_nome" value="<?php echo utf8_encode(@$campo['tx_categoria_nome']);  ?>" /></td>
            </tr>
        </table>
    </div>
    <?php

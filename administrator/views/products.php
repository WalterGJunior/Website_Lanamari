...
if(isset($_POST['cadastrar'])){  // caso nao seja passado o id via GET cadastra 
    ...
    $_POST['tx_categoria'] = utf8_decode($_POST['tx_categoria']);
    $_POST['tx_sub_categoria'] = utf8_decode($_POST['tx_sub_categoria']);

    if($_POST['consulte']){$_POST['tx_valor'] = '0';}

$crud = new crud('tb_produtos');  // instancia classe com as operaçoes crud, passando o nome da tabela como parametro
$crud->inserir("tx_id,tx_nome,tx_categoria,tx_sub_categoria,tx_img,tx_descricao,tx_valor,tx_destaque","'$_POST[tx_id]','$_POST[tx_nome]','$_POST[tx_categoria]','$_POST[tx_sub_categoria]','$_POST[tx_id]','$_POST[tx_descricao]','$_POST[tx_valor]','$_POST[tx_destaque]'"); // utiliza a funçao INSERIR da classe crud        
...
$crud = new crud('tb_produtos'); // instancia classe com as operaçoes crud, passando o nome da tabela como parametro
$crud->atualizar("tx_id='$_POST[tx_id]',tx_nome='$_POST[tx_nome]',tx_categoria='$_POST[tx_categoria]',tx_sub_categoria='$_POST[tx_sub_categoria]',tx_descricao='$_POST[tx_descricao]',tx_valor='$_POST[tx_valor]',tx_destaque='$_POST[tx_destaque]'", "tx_id='$getId'"); // utiliza a funçao ATUALIZAR da classe crud
...
<form action="" method="post" enctype="multipart/form-data"><!--   formulario carrega a si mesmo com o action vazio  -->
    <input type="hidden" name="pag" value="products_list.php?tx_id=<?php echo @$getId; ?>&" /><!--   identifica a pagina de requisição para o crud  -->
            ...
            <tr>
                ...
                <td id="campoPai">
                    ...
                    while($arquivo = $diretorio -> read()){ 
                        ...
                        if ($arquivo!= "." and $arquivo != ".." and $_GET['tx_id']) { 
                    ...
                    <input type='file' size='32' name='my_field[]' value='' /><br><br></td>
                    <input type="hidden" name="action" value="multiple" />
                    <input type="hidden" name="tx_id" value="<?php if(@$getId){echo @$getId;}else{echo @$newId;} ?>" />                     
                </tr>
                <tr>
                    <td class="backgroundCenter">Produto em Destaque?</td>
                    <td>
                        <select  class="campoHidden" required  name="tx_destaque">
                            <?if(@$campo['tx_destaque']){?><option value="<?php echo utf8_encode(@$campo['tx_destaque']);  ?>"><?php echo utf8_encode(@$campo['tx_destaque']);  ?></option><?}?>        
                ...
                <tr>
                    ...
                    <td>
                        <select  class="campoHidden" required  name="tx_categoria">
                            ...
                            <?php
$consulta = mysql_query("SELECT * FROM tb_produtos_categoria ORDER BY tx_categoria_nome"); // query que busca todos os dados da tabela COBRANÇA
while($categoria = mysql_fetch_array($consulta)){ // laço de repetiçao que vai trazer todos os resultados da consulta
    ?>
    <option value="<? echo utf8_encode(@$categoria['tx_categoria_nome']) ?>"><? echo utf8_encode(@$categoria['tx_categoria_nome']) ?></option>
    <?}?>
</select>
</td>
</tr>
    ...
    <tr>
        ...
        <td>
            <select  class="campoHidden" required  name="tx_sub_categoria">
                ...
                <?php
$consulta = mysql_query("SELECT * FROM tb_produtos_sub_categoria ORDER BY tx_sub_categoria_nome"); // query que busca todos os dados da tabela COBRANÇA
while($subcategoria = mysql_fetch_array($consulta)){ // laço de repetiçao que vai trazer todos os resultados da consulta
    ?>
    <option value="<? echo utf8_encode(@$subcategoria['tx_sub_categoria_nome']) ?>"><? echo utf8_encode(@$subcategoria['tx_sub_categoria_nome']) ?></option>
    <?}?>
</select>
</td>
</tr>
<tr>
    <td class="backgroundCenter">Valor do Produto</td>
    <td><input class="campoHidden" required type="text" size="40%"  name="tx_valor" value="<?php echo @$campo['tx_valor'];  ?>" /><br><input class="campoHidden" type='checkbox' value='consulte' name='consulte' <? if($campo['tx_valor'] == 0){echo 'checked';} ?> ><font size=1.8>( Não informar o valor. )</font></td>
</tr>
<tr>
